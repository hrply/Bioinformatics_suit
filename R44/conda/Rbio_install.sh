#!/bin/bash
# R-bio 顺序执行所有 Stage 的包装脚本
#
# 使用方法:
#   conda activate rbio
#   ./Rbio_all_stages.sh --cpu [--china]
#   ./Rbio_all_stages.sh --gpu [--china]
#   ./Rbio_all_stages.sh --stage 5 [--china]  # 从 Stage 5 开始
...
main() {
    ...
    if [ "$START_STAGE" = "gpu" ]; then
        ...
        if [ -z "$START_STAGE" ] || [ "$START_STAGE" -le 1 ] 2>/dev/null; then
            run_stage "${SCRIPT_DIR}/Rbio_stage1.sh" $COMMON_ARGS
        fi
        if [ -z "$START_STAGE" ] || [ "$START_STAGE" -le 2 ] 2>/dev/null; then
            run_stage "${SCRIPT_DIR}/Rbio_stage2.sh" $COMMON_ARGS
        fi
        if [ -z "$START_STAGE" ] || [ "$START_STAGE" -le 3 ] 2>/dev/null; then
            run_stage "${SCRIPT_DIR}/Rbio_stage3.sh" $COMMON_ARGS
        fi
        if [ -z "$START_STAGE" ] || [ "$START_STAGE" -le 4 ] 2>/dev/null; then
            run_stage "${SCRIPT_DIR}/Rbio_stage4.sh" $COMMON_ARGS
        fi
        if [ -z "$START_STAGE" ] || [ "$START_STAGE" -le 5 ] 2>/dev/null; then
            run_stage "${SCRIPT_DIR}/Rbio_stage5.sh" $COMMON_ARGS
        fi
        if [ -z "$START_STAGE" ] || [ "$START_STAGE" -le 6 ] 2>/dev/null; then
            run_stage "${SCRIPT_DIR}/Rbio_stage6.sh" $COMMON_ARGS
        fi
        if [ -z "$START_STAGE" ] || [ "$START_STAGE" -le 7 ] 2>/dev/null; then
            run_stage "${SCRIPT_DIR}/Rbio_stage7.sh" $COMMON_ARGS
        fi
        if [ -z "$START_STAGE" ] || [ "$START_STAGE" -le 8 ] 2>/dev/null; then
            run_stage "${SCRIPT_DIR}/Rbio_stage8.sh" $COMMON_ARGS
        fi
    ...
    echo "  Rscript -e \"library(Seurat); library(Signac); library(ArchR); library(Giotto)\""
}
