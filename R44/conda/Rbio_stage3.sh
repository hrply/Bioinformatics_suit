#!/bin/bash
# Stage 3: Python 包安装 + BSgenome 离线包安装
#
# 使用方法:
#   conda activate rbio2
#   ./Rbio_stage3_python.sh [--china]
#
...
install_bsgenome_from_offline() {
    ...
    local offline_pkg=$(ls "$SCRIPT_DIR"/$pattern 2>/dev/null | head -1)
...
BiocManager::install(c('BSgenome', 'GenomeInfoDb'), ask = FALSE, update = FALSE, site_repository = '${bioc_url}/packages/3.20/bioc');
