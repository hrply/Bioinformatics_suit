...
ENV_NAME="${CONDA_DEFAULT_ENV:-rbio}"
...
setup_china_mirrors() {
    ...
    # 设置代理（用于访问 bioconductor.org 等国外源）
    # 优先使用环境变量 HTTP_PROXY/http_proxy，未设置则跳过
    ...
    if [ -n "$proxy_url" ]; then
        ...
        export HTTP_PROXY="$proxy_url"
        export HTTPS_PROXY="$proxy_url"
        export no_proxy="localhost,127.0.0.1,mirrors.tuna.tsinghua.edu.cn,mirrors.westlake.edu.cn"
...
check_conda_env() {
    if [ -z "$CONDA_PREFIX" ]; then
        ...
        log_error "  conda create -n <env_name> r-base=4.4 python=3.12"
        log_error "  conda activate <env_name>"
        exit 1
    fi
...
check_r() {
    ...
    log_info "R 版本: $R_VERSION"
    
    R_MAJOR=$(echo "$R_VERSION" | cut -d. -f1)
    R_MINOR=$(echo "$R_VERSION" | cut -d. -f2)
    
    if [ "$R_MAJOR" != "4" ] || [ "$R_MINOR" != "4" ]; then
        log_warn "R 版本 $R_VERSION 不符合要求（需要 4.4.x）"
        log_info "强制重装 r-base=4.4.3..."
        mamba install -y -c conda-forge r-base=4.4.3 || {
            log_error "r-base=4.4.3 安装失败"
            exit 1
        }
        R_VERSION=$(R --version | head -1 | awk '{print $3}')
        log_info "R 版本已更新为: $R_VERSION"
    fi
}
...
setup_github_token() {
    ...
    if [ -n "$GITHUB_TOKEN" ]; then
        log_info "从环境变量 GITHUB_TOKEN 读取"
    elif [ -n "$GITHUB_PAT" ]; then
        GITHUB_TOKEN="$GITHUB_PAT"
        log_info "从环境变量 GITHUB_PAT 读取"
    elif [ -f "$HOME/.github_token" ]; then
        GITHUB_TOKEN=$(cat "$HOME/.github_token" | tr -d '\n')
        log_info "从文件 ~/.github_token 读取"
    else
        log_warn "未检测到 GitHub Token，将使用匿名访问（可能受限）"
        log_warn "请通过以下方式配置："
...
local({
  ...
  options(BioCsoft = "https://mirrors.westlake.edu.cn/bioconductor/packages/3.20/bioc")
  options(BioCann = "https://mirrors.westlake.edu.cn/bioconductor/packages/3.20/data/annotation")
  options(BioCexp = "https://mirrors.westlake.edu.cn/bioconductor/packages/3.20/data/experiment")
  options(BioCworkflows = "https://mirrors.westlake.edu.cn/bioconductor/packages/3.20/workflows")
})
EOF
    else
        tee "$r_etc_dir/Rprofile.site" << 'EOF'
local({
  ...
  options(BioCsoft = "https://bioconductor.org/packages/3.20/bioc")
  options(BioCann = "https://bioconductor.org/packages/3.20/data/annotation")
  options(BioCexp = "https://bioconductor.org/packages/3.20/data/experiment")
  options(BioCworkflows = "https://bioconductor.org/packages/3.20/workflows")
})
EOF
    fi
...
verify_installation() {
    ...
    Rscript -e "
packages <- c('Seurat', 'Signac', 'Azimuth', 'ArchR', 'Giotto', 'monocle3')
for (pkg in packages) {
...
可用脚本:
  ...
  Rbio_stage4.sh   - Stage 4: R 依赖包 (CRAN + Bioc)
...
示例:
  ./Rbio_stage1.sh --china
  ./Rbio_stage5.sh --china --password "xxx"

前提条件:
  conda create -n bio r-base=4.4 python=3.12
  conda activate bio
EOF
}
