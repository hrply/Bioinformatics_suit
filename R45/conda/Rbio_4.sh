#!/bin/bash
# Stage 4: R 包安装（CRAN + Bioconductor 3.22）
#
# 使用方法:
#   conda activate bio
#   ./Rbio_4.sh [--china]
...
install_r_packages() {
    ...
    # 先安装基础依赖包（解决依赖链问题）
    log_info "Installing base dependencies first"
    Rscript -e "options(repos = c(cran = '${cran_url}'), download.file.method = 'libcurl');
install.packages(c(
    'fs', 'sass', 'bslib', 'systemfonts', 'textshaping', 'ragg',
    'mvtnorm', 'KernSmooth', 'microbenchmark', 'polyclip',
    'commonmark', 'httpuv', 'sourcetools', 'Cairo',
    'rmarkdown', 'htmlwidgets', 'shiny'
), upgrade = FALSE)" \
        || { log_warn "部分基础依赖安装失败，继续..."; }
...
BiocManager::install(c('Biobase','BiocGenerics','DESeq2','DelayedArray',
    ...
    'multtest','rhdf5','rtracklayer','S4Vectors','SingleCellExperiment',
...
" || log_warn "enrichR 安装失败（非关键包，部分功能可能受限）"
    ...
    Rscript -e "options(repos = c(cran = '${cran_url}'), download.file.method = 'libcurl');
install.packages('ggrepel', upgrade = FALSE)" \
...
main() {
    if [[ "$1" == "--help" ]]; then
        echo "Stage 4: R 包安装（CRAN + Bioconductor ${BIOC_VERSION}）"
        echo ""
